# Onco-tidien — Spécification Produit

**Version** : 1.0
**Date** : 27/03/2026
**Auteur** : Dr. SAYAH Lina / Équipe Hackathon ICL
**Statut** : Brouillon — En attente de validation PO
**Maquettes** : [Médecin](https://indie-rok.github.io/loopy-doo/doctor.html) | [Patient](https://indie-rok.github.io/loopy-doo/patient.html)
...
#### 3.1.4 Tableau de bord patient (suivi)
...
- **Série d'observance** : nombre de jours consécutifs à 100% (voir §3.3)
...
**En-tête** : Nom du patient + jour relatif + date réelle (ex : « J3 — Samedi 17 Oct. »)
...
- Confirmé : carte grisée + coche bleue
...
4. Le patient confirme → **le médicament est injecté dans son planning** :
   ...
   - Visible par le médecin dans le tableau de bord
...
### 3.3 Gamification — Série d'observance
...
- La série s'incrémente de +1 pour chaque jour consécutif complété
- Si un jour est manqué (même 1 seul médicament non confirmé) → **la série revient à 0**
- La série n'est active que les jours où des médicaments sont prévus (les jours sans traitement ne cassent pas la série)
...
**Médecin** :
- Série affichée sur la fiche patient dans le tableau de bord
- Historique : série maximale atteinte, nombre de ruptures
...
| Règle | Déclencheur | Contenu | Moment |
|---|---|---|---|
...
- Cliquer sur une notification ouvre l'application sur la vue Aujourd'hui avec le médicament concerné mis en surbrillance
...
- **Médecin** : bouton « Imprimer la fiche » sur l'écran du code patient + sur le tableau de bord
...
|---|---|---|---|---|
| Aprépitant 125 mg | 2h avant chimio | — | — | — |
| Aprépitant 80 mg | — | Matin | Matin | — |
| Ondansétron 8 mg | 2h avant chimio | — | — | — |
| Solupred 20 mg (2 cp) | — | Matin | Matin | — |
| Solupred 20 mg (1 cp) | — | — | — | Matin |
| Olanzapine 5 mg | — | Soir | Soir | Soir |
...
|---|---|---|
| Cycle ≥ 14 jours (J1-J15) | Pegfilgrastim 6 mg | 1 injection unique à J3 (48h post-chimio) |
...
Si activé : **Doxycycline 100 mg** — 1 comprimé le soir, au moins 30 min avant toute position allongée, tous les jours du cycle.
...
| **Diarrhée** | Diosmectite sachet | 1 sachet | Max 3×/jour, **2h d'écart** avec les autres médicaments | Éviter fruits, fibres, laitages, viandes |
| **Douleur** | Paracétamol gélule | <50 kg : 500 mg / ≥50 kg : 1000 mg | Renouvelable toutes les 6h, max 4×/jour | — |
...
- **Mobile-first** : optimisé pour smartphone (largeur max 430 px), utilisable sur ordinateur
- **Langue** : tout en français
- **Accessibilité** : texte lisible (16 px minimum), contrastes suffisants, zones de toucher larges (48 px)
...
| Fond application | Gris clair | `#F8FAFC` |
...
|---|---|
| Application mobile (patient) | Expo (React Native) |
| Interface web (médecin) | Next.js |
| Base de données | SQLite (locale pour le MVP) |
...
Patient {
  id : chaîne (UUID)
  code_acces : chaîne (6 chiffres)
  nom : chaîne
  prenom : chaîne
  date_naissance : date
  poids : nombre (kg)
  date_heure_chimio : horodatage
  cree_le : horodatage
}

ConfigProtocole {
  id : chaîne
  patient_id : clé étrangère → Patient
  nom_protocole : chaîne
  type_cycle : "J1-J15" | "J1-J8-J15"
  niveau_emetisant : "haut" | "modere" | "faible"
  aplasiant_active : booléen
  type_aplasiant : "Pegfilgrastim" | "Filgrastim 3j" | "Filgrastim 5j" | nul
  mucite_activee : booléen
  folliculite_activee : booléen
}

EntreeAgenda {
  id : chaîne
  patient_id : clé étrangère → Patient
  nom_medicament : chaîne
  posologie : chaîne
  jour_relatif : nombre (1-7+)
  creneau_horaire : "2H_AVANT" | "MATIN" | "MIDI" | "SOIR"
  categorie : "antiemetique" | "aplasiant" | "bain_bouche" | "folliculite" | "symptome"
  source : "genere" | "modif_medecin" | "chatbot_symptome"
  date_heure_reelle : horodatage | nul (calculée après saisie patient)
  confirme_le : horodatage | nul
  est_pris : booléen
}
...
SignalementSymptome {
  id : chaîne
  patient_id : clé étrangère → Patient
  type_symptome : "nausee" | "diarrhee" | "douleur"
  signale_le : horodatage
  medicament_ajoute : chaîne | nul
  conversation_chatbot : JSON
}

Serie {
  patient_id : clé étrangère → Patient
  serie_courante : nombre
  serie_maximale : nombre
  dernier_jour_complete : date
}
```
...
- [ ] Première connexion patient (code + date chimio + poids)
...
- [ ] Tableau de bord médecin (adhésion, symptômes, historique)
- [ ] Gamification série (flamme + animation)
...
- [ ] Paliers série (bronze/argent/or)
...
| 3 | Le chatbot doit-il garder un historique persistant entre sessions ? | Stockage et expérience utilisateur |
| 4 | Hébergement HDS (Hébergeur de Données de Santé) requis pour la production ? | Choix d'infrastructure |
...
- **Interface médecin** : https://indie-rok.github.io/loopy-doo/doctor.html
- **Interface patient** : https://indie-rok.github.io/loopy-doo/patient.html
